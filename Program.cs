...
app.MapGet("/debug-files", (IWebHostEnvironment env) =>
{
    ...
    var indexPath = Path.Combine(homePath, "Index.cshtml");

    var output = new
    {
        env.ContentRootPath,
        ViewsExists = Directory.Exists(viewsPath),
        HomeExists = Directory.Exists(homePath),
        IndexExists = System.IO.File.Exists(indexPath),
        Files = Directory.Exists(homePath)
            ? Directory.GetFiles(homePath).Select(Path.GetFileName).ToArray()
            : Array.Empty<string>()
    };
